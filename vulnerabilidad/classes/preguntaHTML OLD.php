		...
		switch ($tipo) {
			...
			case 'OM':
				...
				foreach($arr_resp as $item){
					...
					$data.= "<input class=\"required\" type=\"radio\" name=\"".$ID_PREGUNTA."\" id=\"".$ID_PREGUNTA."\" value=\"".$datos_res[1]."\" ".$selected." title=\"".($id+1)."\"><label for=\"".$ID_PREGUNTA."\">&nbsp;".str_replace(' ','&nbsp;',$datos_res[0])."&nbsp;</label>";
				}
				$data .= "</p>";
				break;				
			case 'OM_Array':			
